...
function railStateText(it: Item, lang: Lang): string {
  ...
  if (lang === "ja") return `。${component}、${it.railExpanded ? "展開状態" : "折りたたみ状態"}、幅 ${width}dp。${it.railModal ? "モーダル型：展開時はスクラム付きで本文に重ね、レイアウトの占有幅は 96dp のまま" : "非モーダル型：現在の幅だけレイアウトを占有"}。上部のメニューボタンで展開・折りたたみを切り替える`;
...
function itemJa(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case "linearProgress":
      return `${it.wavy ? "波形の" : ""}リニアプログレス（${it.value === undefined ? "不確定" : `${it.value}%`}、トラックの太さ ${it.trackThickness ?? 4}dp）`;
    case "circularProgress":
      return `${it.wavy ? "波形の" : ""}サーキュラープログレス（${it.value === undefined ? "不確定" : `${it.value}%`}、トラックの太さ ${it.trackThickness ?? 4}dp）`;
    case "splitButton":
...
function itemEn(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case "linearProgress":
      return `a ${it.wavy ? "wavy " : ""}linear progress indicator (${it.value === undefined ? "indeterminate" : `${it.value}%`}, ${it.trackThickness ?? 4}dp track thickness)`;
    case "circularProgress":
      return `a ${it.wavy ? "wavy " : ""}circular progress indicator (${it.value === undefined ? "indeterminate" : `${it.value}%`}, ${it.trackThickness ?? 4}dp track thickness)`;
    case "splitButton":
...
function itemZh(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case "linearProgress":
      return `${it.wavy ? "波浪形" : ""}线性进度条（${it.value === undefined ? "不确定进度" : `${it.value}%`}，轨道粗细 ${it.trackThickness ?? 4}dp）`;
    case "circularProgress":
      return `${it.wavy ? "波浪形" : ""}圆形进度条（${it.value === undefined ? "不确定进度" : `${it.value}%`}，轨道粗细 ${it.trackThickness ?? 4}dp）`;
    case "splitButton":
...
function itemKo(it: Item): string {
  ...
  switch (it.kind) {
    ...
    case "linearProgress": return `${it.wavy ? "물결 모양 " : ""}선형 진행 표시기(${it.value === undefined ? "불확정" : `${it.value}%`}, 트랙 두께 ${it.trackThickness ?? 4}dp)`;
    case "circularProgress": return `${it.wavy ? "물결 모양 " : ""}원형 진행 표시기(${it.value === undefined ? "불확정" : `${it.value}%`}, 트랙 두께 ${it.trackThickness ?? 4}dp)`;
...
const WIDE_RAIL_STYLE: Record<Lang, string> = {
  ja: "M3 Expressive ナビゲーションレール: 折りたたみ時は幅 96dp、アイコンの下にラベル。展開時は幅 220dp、高さ 56dp の項目内でアイコンとラベルを横並びにし、間隔は 8dp。既存のトップアプリバーに合わせ、両モードの開閉状態すべてで背景は surfaceContainer。選択項目は secondaryContainer のピル型インジケータ、アイコンは onSecondaryContainer、ラベルは secondary。上部のメニューボタンで開閉する。非モーダル型は本文の横に配置し、モーダル型は展開時にスクラムとともに本文に重ね、背景操作を遮断する。スクラムのタップまたは Escape で閉じる。",
  en: "M3 Expressive navigation rail: 96dp wide when collapsed, with labels below icons. Expanded width is 220dp, with 56dp-high destinations and horizontal icon/label rows separated by 8dp. Match the existing top app bar with a surfaceContainer background in both modes, whether collapsed or expanded. The selected destination uses a secondaryContainer pill, onSecondaryContainer icon, and secondary label. A top menu button toggles expansion. The non-modal variant sits beside the content; the modal variant overlays it with a scrim when expanded and blocks background interaction. Dismiss with a scrim tap or Escape.",
  zh: "M3 Expressive 侧边导航栏：折叠宽 96dp，标签位于图标下方。展开宽 220dp，项目高 56dp，图标与标签横向排列，间距 8dp。沿用现有顶部应用栏配色，两种模式在折叠与展开时均使用 surfaceContainer 背景。选中项用 secondaryContainer 胶囊指示器，图标为 onSecondaryContainer，文字为 secondary。顶部菜单按钮切换展开与折叠。非模态型位于内容旁；模态型展开时带遮罩覆盖内容并阻止背景交互，点击遮罩或按 Escape 关闭。",
  ko: "M3 Expressive 내비게이션 레일: 접으면 너비 96dp, 아이콘 아래에 레이블을 배치한다. 펼치면 너비 220dp, 항목 높이 56dp, 아이콘과 레이블을 8dp 간격으로 가로 배치한다. 기존 상단 앱 바와 맞추어 두 모드의 접힌 상태와 펼친 상태 모두 surfaceContainer 배경을 사용한다. 선택 항목은 secondaryContainer 알약 표시기, onSecondaryContainer 아이콘, secondary 레이블을 사용한다. 상단 메뉴 버튼으로 펼치기와 접기를 전환한다. 비모달은 콘텐츠 옆에 배치하고 모달은 펼칠 때 스크림과 함께 콘텐츠를 덮어 배경 조작을 차단한다. 스크림을 탭하거나 Escape를 누르면 닫힌다.",
};
...
const STYLE_NOTES: Record<Lang, Partial<Record<Kind | "boxSheet", string>>> = {
  ja: {
    ...
    linearProgress: "リニアプログレス: 指定された 4dp または 8dp の太さで、端を丸くする。波形指定のときは M3 Expressive の wavy スタイルにする。トラックは secondaryContainer、進捗は primary。",
    circularProgress: "サーキュラープログレス: 指定された 4dp または 8dp の太さで、端を丸くする。波形指定のときは M3 Expressive の wavy スタイルにする。",
    splitButton:
  ...
  en: {
    ...
    linearProgress: "Linear progress: use the specified 4dp or 8dp track thickness with round caps, and the M3 Expressive wavy style when specified. Track is secondaryContainer, progress is primary.",
    circularProgress: "Circular progress: use the specified 4dp or 8dp track thickness with round caps, and the M3 Expressive wavy style when specified.",
    splitButton:
  ...
  zh: {
    ...
    linearProgress: "线性进度条：使用指定的 4dp 或 8dp 轨道粗细和圆形端帽。指定波浪形时使用 M3 Expressive 的 wavy 样式。轨道为 secondaryContainer，进度为 primary。",
    circularProgress: "圆形进度条：使用指定的 4dp 或 8dp 轨道粗细和圆形端帽。指定波浪形时使用 M3 Expressive 的 wavy 样式。",
    splitButton:
  ...
  ko: {
    ...
    linearProgress: "선형 진행 표시기: 지정된 4dp 또는 8dp 트랙 두께와 둥근 끝을 사용한다. 지정된 경우 M3 Expressive 물결 스타일을 사용하며 트랙은 secondaryContainer, 진행은 primary로 표시한다.",
    circularProgress: "원형 진행 표시기: 지정된 4dp 또는 8dp 트랙 두께와 둥근 끝을 사용한다. 지정된 경우 M3 Expressive 물결 스타일을 사용한다.",
